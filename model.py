...
from keras.layers import Input, CuDNNGRU, Bidirectional, LSTM, Dot, Softmax
...
id2seq_file = '../../../protein.dictionary.tsv'
...
ds_file = '../../../yeast/Human,E.coli,Drosophila.tsv'
label_index = 2
...
def build_model():
    ...
    r1 = Bidirectional(LSTM(hidden_dim, return_sequences=True))  
    l2 = Conv1D(hidden_dim, 3)
    r2 = Bidirectional(LSTM(hidden_dim, return_sequences=True))  
    l3 = Conv1D(hidden_dim, 3)
    r3 = Bidirectional(LSTM(hidden_dim, return_sequences=True))  
    l4 = Conv1D(hidden_dim, 3)
    r4 = Bidirectional(LSTM(hidden_dim, return_sequences=True))  
    l5 = Conv1D(hidden_dim, 3)
    r5 = Bidirectional(LSTM(hidden_dim, return_sequences=True))  
    ...
    for _ in range(num_heads):
        attention_layer = Dot(axes=[2, 2])([s1, s2])
        attention_weights = Softmax()(attention_layer)
        attended_s1 = Dot(axes=[2, 1])([attention_weights, s2])
attended_s2 = Dot(axes=[2, 1])([attention_weights, s1])
        attention_heads.append(attended_s1)
        attention_heads.append(attended_s2)

    merge_layer = concatenate(attention_heads, axis=2)
    merge_layer = BatchNormalization()(merge_layer)
    merge_layer = Dense(128, activation='relu')(merge_layer)
    merge_layer = BatchNormalization()(merge_layer)
    merge_layer = Dropout(0.5)(merge_layer)

    output_layer = Dense(1, activation='sigmoid')(merge_layer)

    model = Model(inputs=[seq_input1, seq_input2], outputs=output_layer)
